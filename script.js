...
function drawGameBoard() {
    const lightGreen = 'rgb(1, 161, 1)';
    const darkGreen = 'rgb(2, 44, 2)';

    for (let i = 0; i < canvas.height / gridSize; i++) {
        for (let j = 0; j < canvas.width / gridSize; j++) {
            if (!isOdd(i) && isOdd(j)) {
                drawTile(j, i, darkGreen);
            }
            if (!isOdd(i) && !isOdd(j)) {
                drawTile(j, i, lightGreen);
            }
...
function growSnake() {
    const newPart = { x: snake.body[1].x, y: snake.body[1].y }
    return snake.body.push(newPart);
}
